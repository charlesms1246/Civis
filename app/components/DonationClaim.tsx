...
interface DonationClaimProps {
  proofTypeId: ProofTypeId
}

export function DonationClaim({ proofTypeId }: DonationClaimProps) {
  ...
  const chainId = process.env.NEXT_PUBLIC_CHAIN_ID || '102031'
  const { status: txStatus, isPolling } = useTransactionStatus(
    mintResult?.transactionHash || null, 
    chainId
  )
  
  // tCTC balance check for gas warning
  const { data: tctcBalance } = useBalance({ address: address as `0x${string}` | undefined })
  const hasSufficientGas = tctcBalance ? parseFloat(tctcBalance.formatted) >= 0.01 : true
      ...
      if (response.ok) {
        ...
        if (nftData) {
          return {
            ...
            contractAddress,
            name: nftData.metadata?.name || 'Civis Proof NFT',
        ...
        try {
          ...
          if (metadataResponse.ok) {
            ...
            return {
              ...
              contractAddress,
              name: metadata.name || 'Civis Proof NFT',
      ...
      return (
        <div className="alert alert-warning mb-6">
          ...
          <div>
            ...
            <p className="text-sm">Please connect your wallet to Creditcoin Testnet using the button in the top right to continue</p>
          </div>
        </div>
      )
    }
      ...
      return (
        <div className="alert alert-error mb-6">
          ...
          <div>
            ...
            <p className="text-sm">
              Please switch to Creditcoin Testnet (Chain ID: 102031) to continue. You are currently on chain ID {currentChainId}.
            </p>
          </div>
          <button 
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-base-content/70">{config.description}</p>
        </div>
      </div>
        ...
        <div className="card-body">
          {currentStep === 'instructions' && (
            <div className="space-y-6">
              {isConnected && !hasSufficientGas && (
                <div className="alert alert-warning mb-4">
                  <AlertCircle className="w-5 h-5" />
                  <div>
                    <h4 className="font-bold">Low tCTC Balance</h4>
                    <p className="text-sm">
                      You need at least 0.01 tCTC to pay gas fees for proof submission.
                      Visit the <a href="/shop" className="link link-primary">Civis Shop</a> to top up,
                      or get tCTC from the{' '}
                      <a href="https://creditcoin-testnet.blockscout.com" target="_blank" rel="noopener noreferrer" className="link link-primary">testnet faucet</a>.
                    </p>
                  </div>
                </div>
              )}
              <div className="flex items-center gap-4 mb-6">
                <div className="text-6xl">{config.nftEmoji}</div>
                <div>
                  <h2 className="card-title text-2xl">{config.name}</h2>
              ...
              <div className="alert alert-info">
                ...
                <div>
                  ...
                  <div className="text-sm">
                    {config.donateInstructions} {config.emailInstructions}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-base-200 p-4 rounded-lg">
                  <ol className="list-decimal list-inside space-y-2 text-sm">
                    <li>Visit <a href={config.donateUrl} target="_blank" rel="noopener noreferrer" className="link link-primary">{config.donateUrl.replace('https://', '')}</a></li>
                    <li>Choose your donation amount (any amount qualifies for a Civis proof)</li>
                    <li>In the donation form, use your email address so you receive a confirmation: <code className="bg-base-300 px-1 rounded text-xs">{address || '0x...'}</code></li>
                    <li>Complete the donation process</li>
                    <li>Check your email for the donation confirmation from <strong>{config.emailDomain}</strong></li>
                  </ol>
                </div>
                ...
                <div className="alert alert-warning">
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      ...
                      Only the donation amount and your wallet address are verified on-chain.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="card-actions justify-between">
                ...
                >
                  ...
                  Donate to {config.name.replace(' Donation', '')}
                </a>
                <button 
          ...
          {currentStep === 'upload' && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-base-content/70">
                  Upload your {config.name.toLowerCase()} confirmation email (.eml file) for vlayer verification
                </p>
              </div>
            ...
            <div className="space-y-6 text-center">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <div className="text-lg font-semibold text-primary">
                      {config.name}: ${emailProofResult.donationAmount}
                    </div>
                  </div>
                  <div className="flex justify-between">
              ...
              {isWalletConnected && !isOnCorrectChain && mounted && (
                <div className="alert alert-error max-w-md mx-auto">
                  ...
                  <div>
                    <h4 className="font-bold">Wrong Network</h4>
                    <p className="text-sm">Please switch to Creditcoin Testnet to mint the NFT</p>
                  </div>
                  <button 
                    onClick={handleSwitchChain}
                    disabled={isSwitchingChain}
                ...
                {isLoadingNFT && !enhancedNFTDetails ? (
                  ...
                  <div className="card bg-gradient-to-br from-primary/20 to-secondary/20 border-2 border-primary/40 shadow-lg">
                    <div className="card-body items-center py-8">
                      ...
                      <div className={`text-6xl mb-4 ${(enhancedNFTDetails?.image || nftDetails?.image) ? 'hidden' : ''}`}>{config.nftEmoji}</div>
                  ...
                  <div className="card bg-gradient-to-br from-primary/20 to-secondary/20 border-2 border-primary/40 shadow-lg">
                    <div className="card-body items-center py-8">
                      <div className="text-6xl mb-4">{config.nftEmoji}</div>
                      <h2 className="text-2xl font-bold text-primary mb-2">
                        Civis Proof NFT
                      </h2>
